  ...
  return !products.length ||
    ...
    <div className={style.container}>
      {window.innerWidth <= 920 ? (
        <Popup
          trigger={<FontAwesomeIcon icon={faArrowDownWideShort} className={style.menuFilter} />}
          nested
          position="right top"
          on="click"
        ...
        >
          {(close) => (
            <div>
              <div className={style.filters} style={{display: "flex", flexDirection: "column", alignItems: "flex-start", height: "200px", padding: "2rem"}}>
      ...
      <div className={style.pagination}>
        <div className={style.text}>
          {`Showing ${
            results < products.length
              ? `${currentPage === 1 ? 1 : results * currentPage - 1} - ${
                  results * currentPage
                }`
              : products.length
          } of ${products.length}`}
        ...
        <div className={style.pages}>
          <button onClick={(e) => previousPage(e)} disabled={currentPage === 1}>
          ...
          {pageNumbers?.slice(i, j).map((number) => {
            return (
              <span
                key={number}
                className={
                  number === currentPage ? style.activePage : style.inactivePage
                }
              >
                {number}
              </span>
